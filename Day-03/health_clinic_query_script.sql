...
-- Display all doctors of a department
CREATE PROCEDURE GetDoctorsByDepartment(IN deptId INT)
BEGIN
    SELECT doctor_id,
           doctor_name,
           specialization,
...
-- Display patients from a particular city
...
CALL GetPatientsByCity('Lucknow');


DELIMITER $$

-- Count appointments of a doctor
CREATE PROCEDURE GetAppointmentCount(IN docId INT)
BEGIN
...
CALL GetAppointmentCount(1);


DELIMITER $$

-- Increase doctor's salary
CREATE PROCEDURE IncreaseDoctorSalary(
    IN docId INT,
...
CALL IncreaseDoctorSalary(3, 5000);
...
CREATE TABLE doctor_salary_log (
...
CREATE TRIGGER trg_doctor_salary_update
AFTER UPDATE ON doctors
FOR EACH ROW
BEGIN
    IF OLD.salary <> NEW.salary THEN
        INSERT INTO doctor_salary_log
        (doctor_id, old_salary, new_salary)
        VALUES
        (OLD.doctor_id, OLD.salary, NEW.salary);
    END IF;
END$$
...
CREATE TRIGGER trg_check_medicine_stock
...
CREATE TRIGGER trg_default_appointment_status
